...
* [`src/scrambler/`](src/scrambler/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/scrambler/core.cljs): contains the SPA entry point, `init`
* [`test/scrambler/`](test/scrambler/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `scrambler` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/scrambler/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns scrambler.example
  (:require [scrambler.config :as config])
